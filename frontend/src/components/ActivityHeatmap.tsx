...
export default function ActivityHeatmap({ dailyGoal }: ActivityHeatmapProps) {
    ...
    const calendarDays = useMemo(() => {
        ...
        logs.forEach(log => logsMap.set(log.date, log.calories));
        ...
        for (let i = 1; i <= daysInMonth; i++) {
            ...
            const calories = logsMap.get(dayString);
    ...
    return (
        <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm relative overflow-hidden z-0 isolate min-h-[350px]">
            ...
            <div className="relative">
                {isLoading && (
                    <div className="absolute inset-0 z-20 flex items-center justify-center bg-white/80 transition-opacity">
                ...
                {!isGoalSet && (
                    <div className="absolute inset-0 z-[1] flex items-center justify-center bg-white/60 backdrop-blur-[2px] rounded-lg">
                        <div className="bg-white px-4 py-2 rounded-full shadow-sm border border-gray-200 text-sm font-medium text-gray-600 text-center">
                            Set a daily goal <br /> to see heatmap
                        </div>
                    </div>
                )}
                    ...
                    {calendarDays.map((day, index) => (
                        <div
                            ...
                            className={`
                                ...
                                ${day.date && isGoalSet && day.calories !== null ? 'hover:scale-105 cursor-pointer shadow-sm' : ''}
                            `}
                        >
                            {day.date && isGoalSet && (
                                <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-30 transition-opacity shadow-lg">
